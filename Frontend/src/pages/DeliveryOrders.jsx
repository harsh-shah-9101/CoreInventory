...
const DeliveryOrders = () => {
  ...
  const [form, setForm]           = useState({ productId: '', warehouseId: '', source_location_id: '', qty: '', scheduledDate: new Date().toISOString().split('T')[0] });
          ...
          <div className="card" style={{ marginBottom: '20px', padding: '20px' }}>
            ...
            <form onSubmit={handleCreate} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div>
                ...
                <select value={form.warehouseId} onChange={e => setForm(f => ({ ...f, warehouseId: e.target.value, source_location_id: '' }))} required>
                  <option value="">Select Warehouse…</option>
                  {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
              </div>
              <div>
                <label className="form-label">Source Location (Optional)</label>
                <select value={form.source_location_id} onChange={e => setForm(f => ({ ...f, source_location_id: e.target.value }))} disabled={!form.warehouseId}>
                  ...
                  {filteredOps.map((op, i) => {
                    ...
                    return (
                      <tr key={op.id || i}>
                        ...
                        <td className="muted">{op.source_location_name || '—'}</td>
